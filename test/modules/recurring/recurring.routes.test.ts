...
describe("Rutas de Configuración de Rondas (Recurrencia) - Integración Total", () => {
  ...
  beforeAll(async () => {
    // 1. Crear Zona via HTTP
    const zoneRes = await request(app)
      .post("/api/v1/zones")
      .send({ name: `Zona para Ronda HTTP ${Date.now()}` });
    createdZoneId = zoneRes.body.data.id;

    // 2. Crear Ubicaciones via HTTP
    const loc1Res = await request(app)
      .post("/api/v1/locations")
      .send({ name: `Punto A HTTP ${Date.now()}`, zoneId: createdZoneId });
    const loc2Res = await request(app)
      .post("/api/v1/locations")
      .send({ name: `Punto B HTTP ${Date.now()}`, zoneId: createdZoneId });
    createdLocationIds = [loc1Res.body.data.id, loc2Res.body.data.id];

    // 3. Crear Guardia via HTTP
  ...
  afterAll(async () => {
    ...
    if (createdRecurringId)
      await request(app).delete(`/api/v1/recurring/${createdRecurringId}`);
    if (createdGuardId)
      await prismaClient.user
        .delete({ where: { id: createdGuardId } })
        .catch(() => {});
    if (createdZoneId)
      await prismaClient.zone
        .delete({ where: { id: createdZoneId } })
        .catch(() => {});
  });
  ...
  describe("Flujo Completo de Configuración de Ronda", () => {
      ...
      const payload = {
        ...
        locations: [
          ...
          },
          {
            ...
            tasks: [{ description: "Limpiar sensor", reqPhoto: false }],
          },
        ],
      };
    ...
    it("debe recuperar la configuración completa con todos sus hijos", async () => {
      const response = await request(app).get(
        `/api/v1/recurring/${createdRecurringId}`,
      );
      ...
      const locWithTasks = config.recurringLocations.find(
        (rl: any) => rl.locationId === createdLocationIds[0],
      );
    ...
    it("debe permitir actualizar la ronda (quitar un guardia y cambiar tareas)", async () => {
      const updatePayload = {
        ...
        locations: [
          {
            locationId: createdLocationIds[0],
            tasks: [{ description: "Tarea nueva única", reqPhoto: true }],
          },
        ],
      };
    ...
    it("debe poder consultarse mediante el datatable", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(
        response.body.data.rows.some((r: any) => r.id === createdRecurringId),
      ).toBe(true);
    });
    ...
    it("debe realizar un soft delete de la configuración", async () => {
      ...
      await prismaClient.recurringConfiguration.update({
        where: { id: createdRecurringId },
        data: { softDelete: false, active: true },
      });
    });
  });
  ...
  describe("Flujo de Ejecución de Ronda (Guardia)", () => {
    ...
    it("debe iniciar una ronda basada en la configuración creada", async () => {
      const response = await request(app).post("/api/v1/rounds/start").send({
    ...
    it("debe registrar un escaneo de punto de control (Kardex)", async () => {
      const response = await request(app).post("/api/v1/kardex").send({
    ...
    it("debe mostrar el escaneo en el detalle/línea de tiempo de la ronda", async () => {
      const response = await request(app).get(
        `/api/v1/rounds/${activeRoundId}`,
      );

      expect(response.status).toBe(200);
      expect(
        response.body.data.timeline.some((t: any) => t.type === "SCAN"),
      ).toBe(true);
    ...
    it("debe finalizar la ronda correctamente", async () => {
      const response = await request(app).put(
        `/api/v1/rounds/${activeRoundId}/end`,
      );
    ...
    it("debe verificar en el historial que la ronda está FINALIZADA", async () => {
      const response = await request(app).get(
        "/api/v1/rounds?status=COMPLETED",
      );
